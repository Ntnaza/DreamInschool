...
export default function ProkerClient({ initialData, divisions }: { initialData: any[], divisions: string[] }) {
  ...
  const handleSave = async () => {
    if (!formState.title) return showToast("Nama kegiatan wajib diisi!", "warning");
    setIsSaving(true);
    ...
    try {
        const result = isEditing ? await updateProgramKerja(formData) : await createProgramKerja(formData);
        if (result.success) {
    ...
    <div className="relative h-full flex flex-col font-sans">
      ...
      <div className="flex-shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
          ...
          <button onClick={openNewModal} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-600/20 flex items-center gap-2 transition-transform active:scale-95 tour-buat-baru-btn text-xs">
            <Plus size={18} /> Proker Baru
          </button>
        </div>
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
               ...
               <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-2xl bg-white dark:bg-[#0f172a] rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-white/10 flex flex-col max-h-[90vh]">
                  ...
                  <div className="p-8 overflow-y-auto flex-1 space-y-5 custom-scrollbar">
                     <div className="flex items-center gap-6 p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-dashed border-slate-300 dark:border-white/10">
                        <div onClick={() => fileInputRef.current?.click()} className="w-24 h-24 rounded-lg bg-white dark:bg-black/20 flex items-center justify-center cursor-pointer overflow-hidden relative group border border-slate-200 dark:border-white/10">
                           {formState.image ? <Image src={formState.image} alt="Preview" fill className="object-cover" /> : (
                              <div className="text-center text-slate-400 group-hover:text-blue-500 transition-colors">
                                 <ImageIcon size={24} className="mx-auto mb-1"/><span className="text-[9px]">Upload</span>
                              </div>
                           )}
                        </div>
                        <div className="flex-1">
                           ...
                           <div className="flex gap-2">
                              ...
                              <div onClick={() => setFormState({...formState, isFeatured: !formState.isFeatured})} className={`px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1 cursor-pointer select-none transition-all ${formState.isFeatured ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' : 'bg-slate-100 text-slate-400 border border-transparent'}`}><Star size={12} fill={formState.isFeatured ? "currentColor" : "none"} /> Unggulan?</div>
                           </div>
                        </div>
                     </div>

                     <div className="space-y-4">
                        <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Nama Kegiatan</label><input type="text" value={formState.title} onChange={e => setFormState({...formState, title: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 outline-none focus:border-blue-500 text-sm font-bold dark:text-white" placeholder="Contoh: LDKS 2026" /></div>
                        <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block flex items-center gap-1"><FileText size={12}/> Deskripsi Singkat</label><textarea rows={2} value={formState.description} onChange={e => setFormState({...formState, description: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 outline-none focus:border-blue-500 text-sm font-medium dark:text-white resize-none" placeholder="Jelaskan tujuan kegiatan secara singkat..." /></div>
                  ...
                  <div className="p-6 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 flex justify-end gap-3">
                     <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">Batal</button>
                     <button onClick={handleSave} disabled={isSaving} className="px-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold shadow-lg shadow-blue-500/30 flex items-center gap-2">
                        {isSaving && <Loader2 className="animate-spin" size={16}/>}{isSaving ? "Menyimpan..." : (isEditing ? "Update Proker" : "Simpan Proker")}
                     </button>
                  </div>
               </motion.div>
            </div>
...
function ProkerCard({ data, onDelete, onEdit, isFirstPriority }: any) {
   ...
   return (
      <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -4, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.1)" }} className="bg-white dark:bg-[#1e293b] p-4 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm cursor-grab active:cursor-grabbing group relative overflow-hidden">
         {data.isFeatured && <div className="absolute top-0 right-0 bg-yellow-400 w-6 h-6 flex items-center justify-center rounded-bl-xl shadow-sm z-10"><Star size={10} className="text-yellow-900" fill="currentColor"/></div>}
         <div className="flex justify-between items-start mb-3">{getStatusBadge(data.progress)}<div className="flex gap-2">{data.image && <ImageIcon size={14} className="text-blue-400" />}<span className="text-[10px] font-bold text-slate-400">{divisi}</span><button onClick={() => onEdit(data)} className="text-slate-300 hover:text-blue-500 transition-colors tour-edit-action" title="Edit"><Edit size={14} /></button><button onClick={() => onDelete(data.id)} className="text-slate-300 hover:text-red-500 transition-colors" title="Hapus"><Trash2 size={14} /></button></div></div>
         ...
         <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-3 line-clamp-2 leading-relaxed">{deskripsi}</p>
         <div className="mb-4"><div className="flex justify-between text-[10px] font-bold text-slate-400 mb-1"><span>Progress</span><span>{data.progress}%</span></div><div className="w-full h-1.5 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${statusColor}`} style={{ width: `${data.progress}%` }} /></div></div>
